...
class trocrDataset(Dataset):
    ...
    """
    def __init__(self, paths, processor, max_target_length=128, transformer=transform.RandomRotation(30)):
        self.paths = paths
...
class trocrDataset_txt(Dataset):
    """
    trocr 训练数据集处理
    文件数据结构
    /tmp/0/0.jpg #image
    ...
    /tmp/100/10000.txt #text label
    """
    def __init__(self, processor, max_target_length=128, transformer=lambda x:x, ann_path=None):
        # self.paths = paths
        self.processor = processor
        self.transformer = transformer
        self.max_target_length = max_target_length
        ...
        self.data = []
        self.nsamples = 0
        if type(self.ann_path) == list:
            for file in self.ann_path:
            ...
            with open(self.ann_path,"r") as f:
                for line in f:
                    self.data.append(line.replace("\n","").split(","))
            self.nsamples = len(self.data)
        print(f"Start Traning Images with {self.nsamples} Images.")
    ...
    def __getitem__(self, idx):
            inx = idx %  self.nsamples
            try:
                image_file = self.data[idx][0]
                text = self.data[idx][1]
            except:
                print(f"{idx},{self.data[idx]}")
            image = Image.open(image_file).convert("RGB")
            image = self.transformer(image) ##图像增强函数
            pixel_values = self.processor(image, return_tensors="pt").pixel_values
            #labels = encode_text(text, max_target_length=self.max_target_length, vocab=self.vocab)["input_ids"]
